...
class AdPicForm(forms.ModelForm):
	...
	# deleter = forms.ModelChoiceField(queryset=Photoo.objects.all(),empty_label=None)
	class Meta:
		model = Photoo
	...
	def __init__(self, ad, *args, **kwargs):
		...
		if int(mypics.count()) >= int(settings.NUMPHOTOS[ad.adtype-1][1]):
			self.fields['deleter'] = forms.ModelChoiceField(queryset=mypics,label='Select photo to delete.',required=True,)
